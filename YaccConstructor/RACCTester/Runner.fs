...
type RACCTester((*_gotoSet,_items,_startNterms,_ruleToActionMap*)) = class
    let  runTest =     
        ...
        let tables = new TablesLoader(gotoSet,items,startNterms,ruleToActionMap)
        let parser = new Parser(tables)            
    
        let count = ref 0
    
        let testStreams = [Test.test002]//Test.testGen11 //[Test.test021_1]
