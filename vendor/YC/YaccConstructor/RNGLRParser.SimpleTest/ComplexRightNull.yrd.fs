...
let _rnglr_epsilon_t = Yard.Generators.RNGLR.AST.NonTerm (ref [Yard.Generators.RNGLR.AST.Inner (4, [||]) ], ref -1) |> Yard.Generators.RNGLR.AST.chooseSingleAst
let _rnglr_epsilon_yard_start_rule = Yard.Generators.RNGLR.AST.NonTerm (ref [Yard.Generators.RNGLR.AST.Inner (3, [|Yard.Generators.RNGLR.AST.NonTerm (ref [Yard.Generators.RNGLR.AST.Inner (0, [||]) ], ref -1)|]) ], ref -1) |> Yard.Generators.RNGLR.AST.chooseSingleAst
let _rnglr_rule_s = Array.zeroCreate 3
let _rnglr_call_rule_s = Array.zeroCreate 3
...
_rnglr_call_rule_s.[1] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.AST<_>[]) ->
    ()
_rnglr_call_rule_s.[2] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.AST<_>[]) ->
    if !(_rnglr_treenum _rnglr_children.[1]) = -1 then (_rnglr_stack_calls_ind.Push(1);_rnglr_stack_calls_node.Push(_rnglr_children.[1]))
    if !(_rnglr_treenum _rnglr_children.[2]) = -1 then (_rnglr_stack_calls_ind.Push(0);_rnglr_stack_calls_node.Push(_rnglr_children.[2]))
    if !(_rnglr_treenum _rnglr_children.[3]) = -1 then (_rnglr_stack_calls_ind.Push(0);_rnglr_stack_calls_node.Push(_rnglr_children.[3]))
    ()
