...
module gr =
    ...
    let mutable clusterVertex : string list list = [] 
    let mutable outPath = ""
    ...
    inherit Generator()     
        ...
        override this.Generate (definition, args) =
            ...
            for prod in grammar.production do
                ...
                getVert'([], res)
                gr.clusterVertex <- gr.vertex :: gr.clusterVertex
                gr.vertex <- []
            ...
            let clustGraph = new ClusteredAdjacencyGraph<string, Edge<string>>(graph);
            for vertex_cluster in gr.clusterVertex do
               let cluster = clustGraph.AddCluster();
