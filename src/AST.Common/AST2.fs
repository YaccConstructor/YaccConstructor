    ...
    interface INode
        ...
        interface System.IComparable with
            member x.CompareTo n =
                match n with
                | :? AST as a -> compare x.pos a.pos
                | _ -> invalidArg "yobj" "cannot compare values of different types"
        new (l, r, p) = {LeftChild = l; RightChild = r; Position = p; pos = -1}
    end
...
//        use out = new System.IO.StreamWriter (path : string)
//        out.WriteLine("digraph AST {")
//        let inline packLabel rule position = (int32 rule <<< 16) ||| int32 position
...
//                else "style=bold,width=10,"
//            out.WriteLine ("    " + b.ToString() + " -> " + e.ToString() + " [" + bold + "label=\"" + label + "\"" + "]")
//        let createTerm t index =
//            createNode index false AstNode ("t " + indToString (tokenToNumber tokens.[t]))
//            index
//        let createTerm2 t index =
//            createNode index false AstNode ("t " + indToString (tokenToNumber t))
//           // index
//        let index = ref 0
//     
//        let rec handle (child : obj) (parent : int) = 
//            if child <> null 
//            then
//                index := !index + 1
//                match child with
//                | :? AST as a ->
//                    let label = "ast"  
//                    createNode !index false AstNode ("n " + label)
//                    let parent = !index
//                    index := !index + 1
//                    let family = a.first
//                    createNode !index false Prod ("prod " + family.prod.ToString())
//                    createEdge parent !index true ""
//                    handle family.nodes !index
//                    if a.other <> null then
//                        for family in a.other do
//                            index := !index + 1
//                            createNode !index false Prod ("prod " + family.prod.ToString())
//                            createEdge parent !index true ""
//                            handle family.nodes !index
...
//                    let label = 
//                        if getRule iN.Position < leftSide.Length then "int " + (indToString leftSide.[getRule iN.Position])
//                        else "error"
//                    createNode !index false IntermidiateNode (label)
//                    let mutable current = !index
//                    if iN.LeftChild <> null then 
//                        handle iN.LeftChild !index
//                        createEdge current (current + 1) true ""
//                    if iN.RightChild <> null then
//                        if iN.LeftChild <> null then
//                            handle iN.RightChild !index
//                            createEdge current (current + 2) true ""
//                        else 
//                            handle iN.RightChild !index
//                            createEdge current (current + 1) true ""
//                    
//                | :? Nodes as n -> 
//                    createNode !index false AstNode ("Nodes")
//                    createEdge parent !index true ""
//                    handle n.fst !index
//                    if n.snd <> null
//                    then 
