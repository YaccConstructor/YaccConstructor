...
//            res <- res |> Seq.distinct |> Seq.sort |> Seq.toList
    ...
    /// </summary>
    member this.GetNextTree (unprocessed : Terminal list) filter = 
        let processed = [| 
                            for i = 0 to tokens.Length - 1 do
                                yield not (unprocessed |> List.exists (fun j -> j.TokenNumber = i))
                        |]
        ...
        let handleAST (ast : AST) = 
            ...
            then ast.first
                ...
                let getUnprocessedCount family = 
                    ...
                    let newToksList = 
                        unprocessed |> List.filter (fun leaf -> familyLeaves |> List.exists ((fun x -> (x :?> Terminal).TokenNumber = leaf.TokenNumber)))
                    newToksList.Length
        ...
        let rec processFamily (family : Family) : INode = 
            ...
            let processNode (node : INode) = 
                ...
                | :? AST as ast -> 
                    ...
                    children.Add child
                | :? Terminal as tok -> 
                    processed.[tok.TokenNumber] <- true
                    children.Add node
    ...
    member this.GetForestWithToken tokRange (tokenToRange: 'TokenType -> seq<_>) = 
        ...
        then []
            ...
            let token = new Terminal(tokNumberOption.Value)
            let forestFam = ref []
