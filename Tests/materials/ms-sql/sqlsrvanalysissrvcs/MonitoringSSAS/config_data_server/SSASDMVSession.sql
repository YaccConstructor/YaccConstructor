...
/****** Object:  Table [stage].[SSASDMVSession]    Script Date: 12/23/2008 13:55:37 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[stage].[SSASDMVSession]') AND type in (N'U'))
DROP TABLE [stage].[SSASDMVSession]
GO
...
CREATE TABLE [stage].[SSASDMVSession](
...
/****** Object:  Index [IX_stage.SSASDMVSession]    Script Date: 12/23/2008 13:55:43 ******/
IF  EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[stage].[SSASDMVSession]') AND name = N'IX_stage.SSASDMVSession')
DROP INDEX [IX_stage.SSASDMVSession] ON [stage].[SSASDMVSession] WITH ( ONLINE = OFF )
GO

/****** Object:  Index [IX_stage.SSASDMVSession]    Script Date: 12/23/2008 13:55:43 ******/
CREATE NONCLUSTERED INDEX [IX_stage.SSASDMVSession] ON [stage].[SSASDMVSession] 
(
	[SESSION_ID] ASC,
	[Instance_Name] ASC
