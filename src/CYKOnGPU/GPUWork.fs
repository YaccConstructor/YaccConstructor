...
type GPUWork(rowSize, nTermsCount, extRecTable:_[], extRules, platformName, extNTermsRuleMap:_[]) =

    let command = 
        <@
            fun (grid:_2D) (table:CellData[]) (rules:uint64[]) (*(tableLenNum:_[])*) (rowSizeNum:_[]) (nTermsCountNum:_[]) (lNum:_[]) (nTermsRuleMap:SymbolRuleMapItem[])-> 
    ...
    let nTermsRulesMap = extNTermsRuleMap
    ...
    member this.Run(l) = 
        kernelPrepare d recTable rules (*[|realRecTableLen|]*) rowSizeArray nTermsCountArray [| l |] nTermsRulesMap
