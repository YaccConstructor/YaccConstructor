...
//    let epsilon = "{ None }"
//    let epsilonSym = new Symbol(epsilon, true)
    ...
    let ILtoInternal (il:Yard.Core.IL.Definition.t<_,_>) =
        ...
        {
            rules = il.grammar.[0].rules |> List.map processRule
            termDict = termDict
            nTermDict = nTermDict
...
//        let b = [| [|1; 2;|]; [|3; 4|] |]
//        b.Aggregate("", fun ac el -> 
//            ac + "[|" + el.Aggregate("", 
//                                        fun ac el -> ac + el.ToString() + ";\n" )
//                + "|];")
