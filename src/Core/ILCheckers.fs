...
module YC.Core.Checkers

open YC.Core.IL
open System.Collections.Generic
open YC.Core.Helpers

let private startRulesCount (def:Definition<_,_>) =
    def.grammar |> List.sumBy (fun module' -> List.fold (fun a r -> if r.isStart then a + 1 else a) 0 module'.rules)
...
let GetIncorrectMetaArgsCount (def:Definition<_,_>) =
    ...
    let checkBody module' =
        ...
        let check acc (name : Source) cnt =
            ...
            if cnt = expected then acc else (name, cnt, expected)::acc
...
let IsChomskyNormalForm (def:Definition<_,_>) =
    def.grammar
...
let GetCoincideModuleNames (def:Definition<Source, Source>) =
...
let GetInvalidOpenings (def:Definition<Source, Source>) =
...
let GetUndeclaredNonterminalsList (def:Definition<Source, Source>) =
...
let IsUnusedRulesExists(def:Definition<_,_>) =
...
let sourcesWithoutFileNames (def:Definition<Source,Source>) =
    ...
    let rec processBody = function
        ...
        | PSeq (s,ac,_) -> collectOpt ac @ (s |> List.collect (fun e -> processBody e.rule))
