...
type CYKCore() =
    ...
    let getCellRuleTop (data:CellData) = //(cellContent:ResizeArray<CellData>) i =
        let curRuleNum,_,_,_ = getData data.rData //cellContent.[cellContent.Count - 1 - i].rData
        let curNT,_,_,_,_ = getRule rules.[(int)curRuleNum]
        curNT

    // возвращает i-ые состояние метки, метку и вес массива указанной ячейки
    let getCellData (x:CellData) = //(cellContent:ResizeArray<CellData>) i =
        let _,curlblState,curcl,curcw = getData x.rData //cellContent.[cellContent.Count - 1 - i].rData
    ...
    let recognitionTable (_,_) (s:uint16[]) weightCalcFun =
        ...
        let recTable = Array.init s.Length (fun i -> Array.init (s.Length - i) (fun j -> Array.init 6 (fun _ -> None)))
        ...
        let processRule rule ruleIndex i k l =
            ...
            if c<>0us then
                ...
                if left.[int b].IsSome && (right.[int c].IsSome)
                then
                    let lState1,lbl1,weight1 = getCellData left.[int b].Value //left m
                    let lState2,lbl2,weight2 = getCellData right.[int c].Value //n
                    ...
                    recTable.[i].[l].[int a] <- Some (new CellData(currentElem,uint32 k))
            ...
            (fun ruleIndex rule ->
                for k in 0..(s.Length-1) do                    
                    ...
                    |0us -> if b = s.[k] then
                                ...
                                (recTable.[k].[0]).[int a] <- Some (new CellData(currentElem,0u))                                                                
                    |_ -> ())
    ...
    let recognize ((grules, start) as g) s weightCalcFun =
        ...
        *)
        ...
        let rec out i last = 
            if (i <= last)
            then let state,lbl,weight = getCellData recTable.[0].[s.Length-1].[i].Value 
                 match i with
        ...
        //out 0 lastIndex
        recTable.[0].[s.Length-1] |> Array.choose id |> Array.map (fun d -> let state,lbl,weight = getCellData d in getString state lbl weight) |> String.concat "\n"
