...
module Yard.Generators.HTMLGenerator.Generator
...
let head = Str ("<html>
   <head>
      ...
      <title>HTMLGenerator</title><style type=\"text/css\">
      body
      {
...
let printSeqBrackets2 l_br r_br metaArgs =
    if Seq.isEmpty metaArgs then ""
    else
        let result = ""
        [ for i in metaArgs -> result + "         <tr class=\"RuleRow\">" + "            <td class=\"RuleColumn\"><a name=\""  + i + "\"><b>" + i + "</b></a></td>"] |> System.String.Concat
...
let printProduction =
    ...
    and printProduction wasAlt (production:Production.t<Source.t,Source.t>)  = 
        ...
        // Token
        | PToken source -> Seq.singleton <| Str ( "<span class=\"TerminalSymbol\"><b>" + Source.toString source + "</b></span>")
        // Vanilla rule reference with an optional args list.
        | PRef(source, attr_option) -> Seq.singleton <|  
                                            Str ( "                       <a class=\"NonTerminalSymbol\" href=\"#" 
                                               + (Source.toString source + printArg attr_option) + "\">" 
                                               + (Source.toString source + printArg attr_option) + "</a>" + "\n")
        // expr*
        | PMany many -> printEbnf "*" many
        // Metarule reference like in "a: mr<x> y z"
        | PMetaRef(rule_name, opt_arg, metaArgs) ->
            "                       <a class=\"NonTerminalSymbol\" href=\"#" 
            + Source.toString rule_name + "\">"
            + Source.toString rule_name + "</a>" + "\n"
            + printMetaArgs metaArgs 
            + printArg opt_arg 
            |> Str |> Seq.singleton
...
let printRule isPublicModule (rule : Rule.t<Source.t, Source.t>) =
    let printArgs args =
        ...
        |> String.concat ""
    let startSign = if rule.isStart then "<span class=\"TerminalSymbol\"><b>Start</b></span>" + endl else ""
    let accessModifier =
        if isPublicModule = rule.isPublic then ""
        elif rule.isPublic then "public "
        else "private "
       
    seq {yield Line(seq{yield Str(
                                 "         <tr class=\"RuleRow\">\n"
                                + "            <td class=\"RuleColumn\"><a name=\"" 
                                + rule.name.text + "\"><b>"
                                +  startSign
                                + accessModifier + rule.name.text + "</b></a></td>" 
                                + (rule.metaArgs |> List.map Source.toString |> printSeqBrackets2 "<" ">")
                                + (printArgs rule.args)
                                + "            <td>\n"
...
let generate (input_grammar: Definition.t<Source.t,Source.t>) =
    ...
    let a = String.concat ""  [print tbSeq]
