module YC.Parsing.Common.FSA.Common
...
let removeEpsilonEdges (fsa : InternalFSA) = 
    let epsilonClosure = Array.init (fsa.States.Length) (fun _ -> new HashSet<_>())
...
let findEquivalenceClasses fsa =  
    let inv = Array.init (fsa.States.Length) (fun _ -> new Dictionary<_,ResizeArray<_>>())
    let buildInv() = 
...
let minimizeFSA fsa =
    ...
    let newStatesSets = Array.init (classes.Count) (fun _ -> new HashSet<_>())
