...
let buildAst : (seq<Token> -> ParseResult<Token>) =
    ...
    for (i,t) in small_gotos do
        for (j,x) in t do
            gotos.[i].[j] <- Some  x
    let lists_reduces = [|[]; [1,2]; [0,1]|]
    let small_reduces =
        [|131074; 131073; 196609; 196610; 131074; 196610|]
...
let inline _rnglr_pop (x : ResizeArray<_>) = 
  ...
  x.RemoveAt(pos)
  res
let _rnglr_translate_token = Array.zeroCreate 4
    ...
    ( fun arg ->
      ...
      |> List.map (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.AST<Token>) -> 
          ...
          | Yard.Generators.RNGLR.AST.Inner (_rnglr_number, _rnglr_children) -> _rnglr_rule_s.[_rnglr_index.[_rnglr_number]] _rnglr_children arg
        )
        |> Array.concat
    ...
    ( fun arg ->
      ...
      |> List.map (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.AST<Token>) -> 
          ...
          | Yard.Generators.RNGLR.AST.Inner (_rnglr_number, _rnglr_children) -> _rnglr_rule_yard_start_rule.[_rnglr_index.[_rnglr_number]] _rnglr_children arg
        )
        |> Array.concat
...
_rnglr_translate_token.[0] <- fun (_rnglr_node : Yard.Generators.RNGLR.AST.MultiAST<Token>) -> 
  ...
  | Yard.Generators.RNGLR.AST.NonTerm (_rnglr_multi_ast,num) ->
    if !num = -1 then
      ...
      _rnglr_stack_calls.Add(2,_rnglr_node);
      _rnglr_multi_ast.Value
      |> List.iter (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.AST<Token>) -> 
          match _rnglr_ast with
...
_rnglr_translate_token.[1] <- fun (_rnglr_node : Yard.Generators.RNGLR.AST.MultiAST<Token>) -> 
  ...
  | Yard.Generators.RNGLR.AST.NonTerm (_rnglr_multi_ast,num) ->
    if !num = -1 then
      ...
      _rnglr_stack_calls.Add(3,_rnglr_node);
      _rnglr_multi_ast.Value
      |> List.iter (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.AST<Token>) -> 
          match _rnglr_ast with
...
_rnglr_rule_s.[1] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.MultiAST<Token>[]) -> 
   fun arg ->
    [|
      for r in 
       _rnglr_result_s.Value.[(_rnglr_treenum _rnglr_children.[0]).Value] (arg * 2)
        do
        for _rnglr_var_1 in 
         (match _rnglr_children.[1] with | Yard.Generators.RNGLR.AST.Term (A value) -> [|value|] | _-> failwith "Token A expected") 
          do
          yield ( r )
    |]
_rnglr_rule_yard_start_rule.[0] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.MultiAST<Token>[]) -> 
   fun arg ->
    _rnglr_result_s.Value.[(_rnglr_treenum _rnglr_children.[0]).Value] arg
let translate node = 
  _rnglr_res_count_s := 0
  _rnglr_res_count_yard_start_rule := 0
  _rnglr_stack_calls.Clear()
  _rnglr_stack_res.Clear()
  if !(_rnglr_treenum node) = -1 then _rnglr_stack_calls.Add(1,node)
