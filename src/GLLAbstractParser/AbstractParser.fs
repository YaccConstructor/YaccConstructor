...
type Parser(parser : ParserSourceGLL) = 
    ...
    member this.Initialise(inpt : IParserInput) (needBuildTree : bool) = 
    ...
    member this.Parse (inpt : IParserInput) (needBuildTree : bool) =
        this.Initialise inpt needBuildTree
        this.HandleQ()
    ...
    member this.HandleQ() = 
            ...
            then
                for e in outEdges do
                    ...
                    then
                        ...
                        if y <> !dummy
                        then
                            sppf.AddChildToPartialNonterm e.Target y
        ...
        let create (curContext:ContextFSA<_>) stateToContinue nonterm =        
            ...
            then
                ...
                then 
                    for p in startV.P.SetP do
                        ...
                        then 
                            ...
                            let newIndex = (1<positionInInput>) * getRightExtension (x.getExtension())
                            sppf.AddChildToPartialNonterm curContext.GssVertex y
                ...
                if !buildTree
                then
                    let nonterm = sppf.FindOrCreatePartialNonterminal nonterm curContext.PosInInput
                    let parent = sppf.GetNode stateToContinue curContext.GssVertex.Nonterm curContext.Data (TreeNode nonterm)
                    let x = sppf.Nodes.Item (int <| unpackNode parent)
                    sppf.AddChildToPartialNonterm curContext.GssVertex parent

                addContext curContext.PosInInput nonterm startV !dummy

        let eatTerm (currentContext : ContextFSA<GSSVertex>) nextToken nextPosInInput nextPosInGrammar =
            if !buildTree
            then
                let newR = sppf.FindOrCreateTerminalNode nextToken currentContext.PosInInput nextPosInInput
                ...
                sppf.AddChildToPartialNonterm currentContext.GssVertex y
        ...
        while descriptorsStack.Count <> 0 do
            ...
            // remove current sppf node from childen list
            if !buildTree
            then
                sppf.RemovePartioalNonterminalChild currentDescr.GssVertex currentDescr.Data

            let possibleNontermMovesInGrammar = parser.OutNonterms.[int currentDescr.PosInGrammar]

            /// Current state is final. It seems that first condition is always true when second is true
            if (currentDescr.Data = !dummy)&&(currentDescr.PosInGrammar |> parser.FinalStates.Contains)
            then 
                if !buildTree
                then
                    let eps = sppf.FindOrCreateTerminalNode epsilon currentDescr.PosInInput currentDescr.PosInInput
    ...
    member this.GetPrefixTree() = 
        ...
        let roots = 
            ...
            |> Array.choose (fun pos ->
                ...
                *)
        let allNodes = Array.ofSeq sppf.MostRootedNodes
        ...
        *)
        let edges = 
            allNodes
            |> Array.collect(fun x ->
                this.SPPF.GetPrefixTreeEdges x beginning nodeGenerator parser.IntToString
                )
