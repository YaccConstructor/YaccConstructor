...
let _rnglr_rule_start = Array.zeroCreate 1
let _rnglr_call_rule_start = Array.zeroCreate 1
let _rnglr_res_count_start = ref 0
let _rnglr_result_start = ref <| Array.zeroCreate 0
...
_rnglr_translate_token.[4] <- fun (_rnglr_node : Yard.Generators.RNGLR.AST.AST<Token>) -> 
  match _rnglr_node with
  | Yard.Generators.RNGLR.AST.Term _ -> failwith "Nonterminal start expected, but terminal found" 
  | Yard.Generators.RNGLR.AST.NonTerm (_rnglr_multi_ast,num) ->
    ( 
      _rnglr_multi_ast.Value
      |> List.map (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.Child<Token>) -> 
          match _rnglr_ast with
          | Yard.Generators.RNGLR.AST.Epsilon -> failwith "Nonterm start can't infer epsilon"
          | Yard.Generators.RNGLR.AST.Inner (_rnglr_number, _rnglr_children) -> _rnglr_rule_start.[_rnglr_index.[_rnglr_number]] _rnglr_children 
        )
        |> List.concat
      ) |> (fun res -> _rnglr_result_start.Value.[!num] <- res)
...
_rnglr_translate_token.[1] <- fun (_rnglr_node : Yard.Generators.RNGLR.AST.AST<Token>) -> 
  ...
  | Yard.Generators.RNGLR.AST.NonTerm (_rnglr_multi_ast,num) ->
    if !num = -1 then
      num := !_rnglr_res_count_start
      incr _rnglr_res_count_start;
      ...
      |> List.iter (
        fun (_rnglr_ast : Yard.Generators.RNGLR.AST.Child<Token>) -> 
          match _rnglr_ast with
          | Yard.Generators.RNGLR.AST.Epsilon -> failwith "Nonterm start can't infer epsilon"
          | Yard.Generators.RNGLR.AST.Inner (_rnglr_number, _rnglr_children) -> _rnglr_call_rule_start.[_rnglr_index.[_rnglr_number]] _rnglr_children
        )
...
_rnglr_rule_yard_start_rule.[0] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.AST<Token>[]) ->  
    _rnglr_result_start.Value.[(_rnglr_treenum _rnglr_children.[0]).Value] 
_rnglr_rule_s.[0] <- 
  fun (_rnglr_children : Yard.Generators.RNGLR.AST.AST<Token>[]) ->  
    (
      let _rnglr_cycle_res = ref []
...
let translate node = 
  ...
  _rnglr_result_start := Array.zeroCreate !_rnglr_res_count_start
