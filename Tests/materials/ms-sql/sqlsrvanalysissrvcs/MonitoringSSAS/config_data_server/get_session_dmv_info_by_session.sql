...
/****** Object:  StoredProcedure [ssas].[get_session_dmv_info_by_session]    Script Date: 12/23/2008 17:15:49 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ssas].[get_session_dmv_info_by_session]') AND type in (N'P', N'PC'))
DROP PROCEDURE [ssas].[get_session_dmv_info_by_session]
GO
...
CREATE PROCEDURE [ssas].[get_session_dmv_info_by_session]( 
...
WITH 
	ssas_connections AS
	  ...
	  WHERE connection_start_time BETWEEN CAST(''' + CAST(@start_time AS nvarchar(50)) + ''' AS datetimeoffset(7)) AND CAST(''' + CAST(@end_time AS nvarchar(50)) + ''' AS datetimeoffset(7))
	...
	ssas_sessions AS
	  ...
	  AND session_start_time BETWEEN CAST(''' + CAST(@start_time AS nvarchar(50)) + ''' AS datetimeoffset(7)) AND CAST(''' + CAST(@end_time AS nvarchar(50)) + ''' AS datetimeoffset(7))
...
JOIN ' + @database_name + '.custom_snapshots.ssas_commands
	...
	WHERE CONNECTION_ID = @connection_ID
		ORDER BY SESSION_LAST_COMMAND_START_TIME
    RETURN
END
