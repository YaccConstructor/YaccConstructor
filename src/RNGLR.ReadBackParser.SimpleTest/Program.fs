...
//        Assert.AreEqual(expected, res)
    ...
    let runTest parser parserSource file expected (translate : (Tree<_> -> 'Value) option) = 
        let path = dir + file
    ...
    member test.``1.1 One wrong1``() =
        ...
        runTest parser parserSource file TER_Error None
...
[<EntryPoint>]
let main argv = 
    (new ``RNGLRReadBack parser tests with simple lexer``()).``1.0 One``();
