...
let getKwToken = 
    ...
    let ucis = FSharpType.GetUnionCases (typeof<cykToken>) |> Array.map nameToUnionCtor  |> dict 
    fun (name:string) defaultSourceText ->
        let upperName = "KW_" + name.ToUpperInvariant()
        ucis.TryGetValue' upperName
        |> Option.map (fun ctor ->  ctor [| |] :?>cykToken)
...
let makeIdent notKeyWord (name:string) (startPos, endPos) =
  ...
  if prefix = "@@" then new CYKToken<_,_> (GLOBALVAR,"")//(defaultSourceText)
  //else if prefix = "##" then GLOBALTEMPOBJ(name)
  else if name.[0] = '@' then new CYKToken<_,_> (LOCALVAR,"")//(defaultSourceText)
  //else if name.[0] = '#' then TEMPOBJ(name)
  else if prefix = "%%" then new CYKToken<_,_> (STOREDPROCEDURE,"")//(defaultSourceText)
  else if notKeyWord then new CYKToken<_,_> (IDENT,"")//(defaultSourceText)
  else  match getKwToken name defaultSourceText with
        | Some(kwToken) -> new CYKToken<_,_> (kwToken,"")
        | None -> new CYKToken<_,_> (IDENT,"")//(defaultSourceText)
