...
let getAbsoluteOffset (map : array<_>) line column =
    if line > map.Length - 1  || line < 1 
    then failwith ("Value of Line is out of Map's range")    
    if column > map.[line] - map.[line - 1] - 1 || column < 1 
