namespace YC.Parsing.Common.GLL

open YC.Parsing.Common.GraphInput
open YC.Parsing.Common.ASTGLL
...
type ParserStructures<'TokenType> (currentRule : int)=
    ...
    let setP = new Dictionary<int64, ResizableUsualOne<int<nodeMeasure>>>(500)//list<int<nodeMeasure>>> (500)
