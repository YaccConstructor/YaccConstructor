...
let printTablesReadBack
    (grammar : FinalGrammarDFA) head (tables : TablesReadBack) (moduleName : string) 
    ...
    let printTriple lBr rBr sep (triple : 'a * 'b * 'c) fstPrinter sndPrinter trdPrinter =
        print lBr
        let fst, snd, trd = triple
        fstPrinter fst
        print sep
        sndPrinter snd
        print sep
    ...
    let printTablesToFSharp () =
        ...
        let printTriple triple fstPrinter sndPrinter trdPrinter =  printTriple "" "" ", " triple fstPrinter sndPrinter trdPrinter
        ...
        print "let private dfas = "
        printDfas tables.dfas
