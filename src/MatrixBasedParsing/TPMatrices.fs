...
//                let exp = (log (double x)) / (log 2.) |> floor |> int
        ...
        let createCudaHelper cudaOptions =   
            ...
            let gpu = Gpu.Default
            {
                gpu = gpu
                options = cudaOptions
                mult1 = gpu.AllocateDevice(bufferSize)
                mult2 = gpu.AllocateDevice(bufferSize)
                result = gpu.AllocateDevice(bufferSize)
            }
...
//                    coeff <- coeff - 1
        ...
        let doCublasMultiplications helpers (getTMatrix: _ -> ProbabilityMatrix.T) (getPMatrix: _ -> ProbabilityMatrix.T) matricesSize (fullTasks: _ []) = 
            ...
            let multiplicationResult = 
                if gpuMutex.WaitOne()
                then
                    Gpu.Copy(from1,helpers.mult1)
                    Gpu.Copy(from2,helpers.mult2)
                    
                    
                    let doOne i =
                        let ptrShift = i * matricesCellCount
                        blas.Gemm (  Operation.N, 
                                     Operation.N, 
                                     matricesSize, 
                                     matricesSize, 
                                     matricesSize, 
                    ...
                    let result = Gpu.CopyToHost helpers.result
