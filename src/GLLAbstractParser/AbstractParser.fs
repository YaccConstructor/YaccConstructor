...
let buildAbstractAst<'TokenType> (parser : ParserSourceGLL<'TokenType>) (input : ParserInputGraph<'TokenType>) : ParseResult<_> = 
    ...
    if input.EdgeCount = 0 then
     ...
     //   else
        ...
        let setU = Array.zeroCreate<Dictionary<int<labelMeasure>, Dictionary<int64, ResizeArray<int<nodeMeasure>>>>> (input.VertexCount )///1
        let structures = new ParserStructures(input.VertexCount)
        ...
        let currentLabel = ref <| packLabel currentrule 0 //packLabel without int
        let dummyGSSNode = new Vertex(!currentVertexInInput, int !currentLabel)
        ...
        let currentContext = ref <| new Context(!currentVertexInInput, !currentLabel, !currentGSSNode, dummy) //without *1<labelMeasure>
        ...
        let containsContext (inputVertex : int) (label : int<labelMeasure>) (vertex : Vertex) (ast : int<nodeMeasure>) =
            ...
            let vertexKey = pack vertex.Level (int vertex.NontermLabel)
            if setU.[index] <> Unchecked.defaultof<Dictionary<int<labelMeasure>, Dictionary<int64, ResizeArray<int<nodeMeasure>>>>>
            then
                ...
                if  cond then
                    ...
                    let dict = new Dictionary<int64, ResizeArray<int<nodeMeasure>>>()
                ...
                let dict1 =  new Dictionary<int<labelMeasure>, Dictionary<int64, ResizeArray<int<nodeMeasure>>>>()
                setU.[index] <- dict1
                let dict2 = new Dictionary<int64, ResizeArray<int<nodeMeasure>>>()
        ...
        let containsEdge (b : Vertex) (e : Vertex) ast =
            ...
            if dict1 <> Unchecked.defaultof<Dictionary<int<nodeMeasure>, Dictionary<int, ResizeArray<int>>>>
            then
            ...
            then
                ...
                then
                    ...
                    for tree in arr do
                        ...
                        let y = structures.GetNodeP findSppfNode findSppfPackedNode dummy label ast tree
                ...
                for edge in outEdges do
                    ...
                    for slotLevels in edge.Value do   
                         ...
                         for level in slotLevels.Value do
                            let resTree = structures.GetNodeP findSppfNode findSppfPackedNode dummy (u.NontermLabel*1<labelMeasure>) sppfNodeOnEdge z 
                            let newVertex = new Vertex(level, slot)
                            addContext i (u.NontermLabel*1<labelMeasure>) newVertex resTree
        ...
        and processing () =  
            ...
            then
              ...
              currentN := structures.GetNodeP findSppfNode findSppfPackedNode dummy !currentLabel !currentN !currentR  
...
//                            condition := true
                    ...
                    match curRight with
                        | :? TerminalNode as t ->
                            currentN := structures.GetNodeP findSppfNode findSppfPackedNode dummy !currentLabel !currentR !currentN
