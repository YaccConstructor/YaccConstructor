...
|];
   ...
   DIDToStateMap = dict [|(0,(State 0));(1,(State 1));(2,(State 2));(3,DummyState)|];
   DStartState   = 1;
   DFinaleStates = Set.ofArray [|2|];
   DRules        = Set.ofArray [|{ 
   FromStateID = 0;
   Symbol      = (DSymbol 2);
...
;{ 
   FromStateID = 2;
   Symbol      = Dummy;
   Label       = Set.ofArray [|List.ofArray [|(FATrace (TSmbE 4));(FATrace (TSeqE 5))|]
|];
   ToStateID   = 3;
}
|];
...
let private gotoSet = 
    Set.ofArray [|((0, 0, 1), set [(0, 1)]);((0, 0, 2), set [(1, 0)]);((0, 0, 3), set [(1, 2)]);((1, 0, 2), set [(1, 0)]);((1, 0, 3), set [(1, 2)]);((1, 1, 2), set [(1, 0)]);((1, 1, 3), set [(1, 2)])|]
    |> dict
