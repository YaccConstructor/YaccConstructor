...
type CfgBuilder<'TokenType, 'BackReference when 'BackReference : equality> private () = 
    
    static member BuildCfg (tree : Tree<'TokenType>)
            (parserSource : GeneratedStuffSource<'TokenType, 'BackReference>) 
            (langSource : LanguageSource) 
            (tokToString : 'TokenType -> string) = 
        ...
        let start, finish = graphToCfg <| graphInfo.Build() <| Some parserSource.TokenToString
        new ControlFlow<_, _>(start, finish, intToToken, parserSource, langSource)