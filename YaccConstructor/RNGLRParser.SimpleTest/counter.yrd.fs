...
let translate (_rnglr_tokens_init : seq<Token>) = 
  ...
  let rec _rnglr_translate_token_s = 
     fun (_rnglr_multi_ast : AST<_> list ref) ->
      ...
      |> List.map (
        fun (_rnglr_ast : AST<_>) -> 
          match _rnglr_ast with
          | Epsilon -> failwith "Nonterm s can't infer epsilon"
          | Inner (_rnglr_number, _rnglr_children) -> _rnglr_rule_s.[_rnglr_index.[_rnglr_number]]  _rnglr_children
        )
        |> List.concat
     ...
     fun (_rnglr_children : MultiAST<_>[]) ->
      [
        for _rnglr_var_0 in 
         (match _rnglr_children.[0] with | Term (A value) -> [value] | _-> failwith "Token A expected") 
          do
          for v in  (_rnglr_translate_token_s (getFamily _rnglr_children.[1]))
            do
            yield ( 1 + v )
      ]
    )
  _rnglr_rule_yard_start_rule.[0] <- (
     fun (_rnglr_children : MultiAST<_>[]) ->
      (_rnglr_translate_token_s (getFamily _rnglr_children.[0]))
    )
