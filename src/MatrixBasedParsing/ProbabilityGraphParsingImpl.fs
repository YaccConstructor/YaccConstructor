...
type ProbabilityAleaCudaHandler(_matrixSize:int) =
    let gpu = Gpu.Default
    let mult1 = gpu.AllocateDevice((int)(_matrixSize * _matrixSize))
    let mult2 = gpu.AllocateDevice((int)(_matrixSize * _matrixSize))
    let result = gpu.AllocateDevice((int)(_matrixSize * _matrixSize))
    ...
    member this.multArrays (from1: float[]) (from2: float[]) =      
        aleaCudaMultArrays blas from1 from2 _matrixSize mult1 mult2 result
