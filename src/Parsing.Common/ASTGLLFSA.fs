module YC.Parsing.Common.ASTGLLFSA
...
//                            createEdge currentPair.Num !num false ""
    ...
    member this.GetAllExistingPaths() : seq<int<positionInInput> * int<positionInInput> * int<positionInGrammar>> =
     ...
     member this.GetAllPaths() : Map<int<positionInGrammar>, seq<list<TerminalNode>>> =
        // A map from nonterminals to sequences of paths generated by nonterminal
        let rec interleave xs = seq {
            let xs = Seq.filter (not << Seq.isEmpty) xs
            if not <| Seq.isEmpty xs
            then
                for hd in xs do yield Seq.head hd
                yield! interleave <| Seq.map Seq.tail xs
        }

         let rec cartesianPathProduct (lpaths : seq<list<'T>>) (rpaths : seq<list<'T>>) : seq<list<'T>> =
            let (|Cons|Nil|) xs =
                if Seq.isEmpty xs
                then Nil
                else Cons(Seq.head xs, Seq.tail xs)
            seq {
                match lpaths, rpaths with
                | Nil, _ | _, Nil -> ()
                | Cons(lhead, ltail), Cons(rhead, rtail) ->
                    yield lhead @ rhead
                    let mutable smallerSquare = cartesianPathProduct ltail rtail
                    let mutable rsize = 0
                    let mutable ltail = ltail
                    for ritem in rtail do
                        yield lhead @ ritem
                        let nextRsize, rsmallerSquare =
                            FSharpx.Collections.Seq.splitAt rsize smallerSquare
                        yield! nextRsize
                        smallerSquare <- rsmallerSquare
                        match ltail with
                        | Nil -> ()
                        | Cons(litem, ltail') ->
                            yield litem @ rhead
         ...
         let rec collectAllPathsFromNode (node: INode) : seq<list<TerminalNode>> =
            match node with
            | :? EpsilonNode -> seq[[]]
            | :? TerminalNode when isDummy node -> seq[[]]
            | :? TerminalNode as term -> seq[[term]]
            | :? PackedNode as packed ->
                match isDummy packed.Left, isDummy packed.Right with
                | true, true -> failwith "unsupported node type"
                | true, false -> collectAllPathsFromNode packed.Right
                | false, true -> collectAllPathsFromNode packed.Left
                | _ -> cartesianPathProduct (collectAllPathsFromNode packed.Left) (collectAllPathsFromNode packed.Right)
            | :? NonTerminalNode as nonTerm -> interleave <| nonTerm.MapChildren(collectAllPathsFromNode)
            | :? IntermidiateNode as inter -> interleave <| inter.MapChildren(collectAllPathsFromNode)
            | _ -> failwith "unsupported node type"

         collectAllNonTerminals roots
        |> Seq.groupBy (fun nonTerm -> nonTerm.Name)
        |> Map.ofSeq
        |> Map.map (fun _ -> Seq.map collectAllPathsFromNode >> interleave >> Seq.distinct)
