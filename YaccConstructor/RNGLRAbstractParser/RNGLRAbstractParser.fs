...
let buildAst<'TokenType> (parserSource : ParserSource<'TokenType>) (qGraph : AdjacencyGraph<int, TaggedEdge<_,'TokenType>>) =
    ...
    let TOKEN = new ResizeArray<'TokenType>()
    let mutable q = ref -1
    ...
    qGraph.Edges |> Seq.iter(
                ...
                TOKEN.Add(edg.Tag) |> ignore)
    for q = 0 to TOKEN.Count - 1 do                
                // If input stream is empty or consists only of EOF token
                if (TOKEN.[q].ToString().Equals("null") || parserSource.EofIndex = parserSource.TokenToNumber TOKEN.[q]) && (q = 0) then
                    if parserSource.AcceptEmptyInput then
                        printfn "Success - New null Tree"
                    else
                    printfn "This grammar cannot accept empty string"
                else

                // Currently processed token
                let curToken = ref TOKEN.[q]
                let curNum = ref (parserSource.TokenToNumber TOKEN.[q])
                ...
                let shift num =
                    ...
                    tokens.Add tokens.[q]
                    if q <= (TOKEN.Count - 2) then
                        curToken := TOKEN.[q+1]
                        curNum := parserSource.TokenToNumber TOKEN.[q+1]
                    else
                ...
                if q=0 then ignore <| addVertex startState 0 None
                if not !isEnd then
    ...
    match !root with
            ...
            | Some res -> Success <| new Tree<_>(TOKEN.ToArray(), res, parserSource.Rules)        